...
describe('loader', () => {
  describe('srcSetLoader', () => {
    describe.each(bundlers)('%s', (_name, createCompiler) => {
      ...
      it('should use url variants from custom backend without emitting', async () => {
        const dir = await createFixtureProject(defaultEntry)
        const {
          assets,
          exports
          ...
          backend: () => ({
            * generate(_source: unknown, metadata: {
              format: string
              width: number
              height: number
            }) {
              yield {
                ...
                originMultiplier: 1,
                url: `https://proxy.test/w:${metadata.width}/f:${metadata.format}`
              }
            }
          })
        })
        ...
        expect(assets).toEqual([])
        expect(exports.default).toBe(`https://proxy.test/w:${imageWidth}/f:jpg`)
        expect(exports.srcMap[`jpg${imageWidth}`]).toBe(exports.default)
      })
      ...
      it('should emit the original for a backend enforcing public path', async () => {
        ...
        expect(assets).toEqual([expect.stringMatching(/^image\.[0-9a-f]{8}\.jpg$/) as string])
        expect(exports.default).toBe(`https://proxy.test/plain/https://static.example.com/assets/${assets[0]}`)
      })
